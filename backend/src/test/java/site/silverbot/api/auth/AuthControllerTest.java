...
class AuthControllerTest extends RestDocsSupport {
    ...
    void signup() throws Exception {
        ...
        TokenResponse response = TokenResponse.of("access-token", "refresh-token", 3600000L);
    ...
    void refresh() throws Exception {
        TokenResponse response = TokenResponse.of("new-access", "new-refresh", 3600000L);
    ...
    void robotLogin() throws Exception {
        ...
        TokenResponse response = TokenResponse.accessOnly("robot-access", 3600000L);
        ...
        mockMvc.perform(RestDocumentationRequestBuilders.post("/api/auth/robot/login")
                ...
                .andDo(document("auth-robot-login",
                        ...
                        responseFields(
                                fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("성공 여부"),
                                fieldWithPath("data.accessToken").type(JsonFieldType.STRING).description("액세스 토큰"),
                                fieldWithPath("data.refreshToken").type(JsonFieldType.STRING).optional().description("리프레시 토큰"),
                                fieldWithPath("data.tokenType").type(JsonFieldType.STRING).description("토큰 타입"),
                                fieldWithPath("data.expiresIn").type(JsonFieldType.NUMBER).description("액세스 토큰 만료(ms)"),
                                fieldWithPath("timestamp").type(JsonFieldType.STRING).description("응답 시간")
                        )
