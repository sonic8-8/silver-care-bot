...
- **대상 커밋**:
  ...
  - `5fcaad1` fix(robot): REST Docs params.location 문서화 보강 [Agent 3]
...
| `backend/src/test/java/site/silverbot/api/robot/RobotControllerTest.java` | 수정 | REST Docs 요청/응답 필드에 `params.location` 문서화 추가 |
...
## 테스트 명령어
```bash
# Backend (Agent 3 범위, 최종 검증)
cd backend && ./gradlew test \
  --tests "site.silverbot.api.robot.service.RobotCommandServiceTest" \
...
- `./gradlew test --tests "site.silverbot.api.robot.RobotControllerTest"` ✅ PASS
- 묶음 실행(3개 테스트 동시): ✅ PASS
