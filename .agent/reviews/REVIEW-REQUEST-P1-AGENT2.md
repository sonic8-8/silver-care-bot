...
| `backend/src/test/java/site/silverbot/api/elder/controller/ElderControllerTest.java` | 신규/수정 | REST Docs 포함 컨트롤러 테스트 + fixture 정리 순서 보정 |
| `backend/src/test/java/site/silverbot/api/elder/controller/EmergencyContactControllerTest.java` | 신규/수정 | REST Docs 포함 컨트롤러 테스트 + fixture 정리 순서 보정 |
| `backend/src/test/java/site/silverbot/api/emergency/controller/EmergencyControllerTest.java` | 신규/수정 | REST Docs 포함 컨트롤러 테스트 + FK 정리 순서 보정 |
| `backend/src/test/java/site/silverbot/api/elder/service/ElderServiceTest.java` | 신규/수정 | 서비스 테스트 + FK 정리 순서 보정 |
| `backend/src/test/java/site/silverbot/api/elder/service/EmergencyContactServiceTest.java` | 신규/수정 | 서비스 테스트 + batch delete/flush/clear 적용 |
| `backend/src/test/java/site/silverbot/api/emergency/service/EmergencyServiceTest.java` | 신규/수정 | 서비스 테스트 (재해제 409/다중 PENDING + FK/batch delete/flush/clear) |
...
7. v8 추가 반영: 테스트 fixture 정리 순서 수정(FK/UNIQUE 위반 해소)
8. v9 추가 반영: 남은 4개 테스트 클래스 fixture 정리 완료 후 백엔드 `clean test` 통과

## v9 지시사항 반영 상세
1. `EmergencyControllerTest` FK 위반 해소
   - `EmergencyContactRepository` 주입
   - `deleteAllInBatch()` 순서를 `emergency -> emergency_contact -> robot -> elder -> user`로 보정
2. `EmergencyServiceTest` FK 위반 해소
   - `EmergencyContactRepository` 주입 후 선삭제 추가
   - `EntityManager.clear()` 추가로 영속성 컨텍스트 정리
3. `ElderServiceTest` FK 위반 해소
   - `EmergencyRepository`, `EmergencyContactRepository`, `RobotRepository` 주입
   - 자식 테이블 선삭제 후 `elder/user` 삭제
4. `EmergencyContactServiceTest` UNIQUE 안정화
   - `deleteAllInBatch()` + `EntityManager.flush()` + `EntityManager.clear()` 적용
...
- 현재 로컬 브랜치는 `origin/feature/phase1-elder` 대비 ahead 상태이며, push는 GitHub HTTPS 인증 이슈로 보류 중입니다.

## 테스트 실행 여부
- Backend:
  - `./gradlew clean test` 통과 (`BUILD SUCCESSFUL`, 2026-02-06)
- Frontend:
  - `npm run test -- --run` 통과 (Test Files 3, Tests 9)
